...
class CustomSession(Session):
    """A Custom Flask SQLAlchemy session to intercept bulk save queries"""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def bulk_save_objects(self, objects, *args, **kwargs):
        """Intercepts Sqlachemy `bulk_save_objects` sets `tenant_key`
        ...
        if (
            config_from_env("MULTI_TENANCY_ENABLED", default=False)
            and getattr(g, "token", None) is not None
        ):
